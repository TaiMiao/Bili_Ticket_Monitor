...
REFRESH_INTERVAL = 2  # 票务信息刷新间隔（秒），1秒以下可能会被风控
...
def fetch_data(max_retries=MAX_RETRIES):
    """从Bilibili API获取票务状态，若失败则重试，使用指数退避和抖动策略。"""
    for attempt in range(max_retries):
        try:
            ...
            if attempt > 0:  # 当重试成功时
                print(Fore.GREEN + "\n重连成功")
                show_table(name, tickets)  # 重新打印表格

            return name, tickets

        except requests.RequestException as e:
            handle_request_exception(e, attempt, max_retries)
...
def handle_request_exception(e, attempt, max_retries):
    ...
    if attempt < max_retries - 1:
        backoff = (2 ** attempt) + random.uniform(0, 1)  # 指数退避加随机抖动
...
def show_table(name, table, first_time=False):
    """打印票务状态表，并根据状态进行颜色编码。"""
...
def show_time(stop_event):
    ...
    while not stop_event.is_set():  # 检测停止事件
        ...
        if current_time != last_printed_time:  # 检测时间是否已被清除
...
def monitor(stop_event):
    ...
    # 初次获取票务状态
    name, new_table = fetch_data()
    if new_table is None:
        return

    show_table(name, new_table, first_time)
    ...
    while not stop_event.is_set():  # 检测停止事件
        ...
        if has_changed(last_table, new_table):
            show_table(name, new_table, not last_table)
            last_table = new_table
...
def main():
    ...
    # 创建并启动线程
    time_thread = threading.Thread(target=show_time, args=(stop_event,), daemon=True)
    ticket_thread = threading.Thread(target=monitor, args=(stop_event,), daemon=True)
    ticket_thread.start()
